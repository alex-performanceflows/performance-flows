...
import {
  ...
  ACCENT, SAND, COMPARE_LABEL,
  clipRange, dailyInRange,
} from "./shared";

export function SEOTab({ data }: { data: CoorieData }) {
  const { palette } = useTheme();
  const { range, compareRange, compare } = useDateRange();

  // Ultimo giorno GSC = max(gsc.daily) — evita di mostrare buchi finali dovuti al lag
          ...
          <div style={{ width: "100%", height: 320 }}>
            <ResponsiveContainer>
              <ComposedChart data={chart} margin={{ top: 10, right: 12, bottom: 4, left: 8 }}>
                ...
                <YAxis yAxisId="right" orientation="right" tick={{ fill: SAND, fontSize: 11 }} axisLine={{ stroke: palette.cardBorder }} tickLine={false} tickFormatter={(v) => integer(Number(v))} width={60} />
                ...
                {clippedCompare && (
                  <>
                    ...
                    <Line yAxisId="right" type="monotone" dataKey="prevImp" name="Impression (prec.)" stroke={SAND} strokeWidth={1.5} strokeDasharray="4 4" strokeOpacity={0.55} dot={false} connectNulls />
                  </>
                )}
                <Line yAxisId="left" type="monotone" dataKey="click" name="Click" stroke={ACCENT} strokeWidth={2.5} dot={false} />
                <Line yAxisId="right" type="monotone" dataKey="imp" name="Impression" stroke={SAND} strokeWidth={2.5} dot={false} />
              </ComposedChart>
            </ResponsiveContainer>
          </div>
        )}
...
function GscTable({ rows, palette, isPage }: {
  ...
  const opportunityBg = "rgba(201,184,150,0.14)";
          ...
          {rows.map((r, i) => {
            ...
            return (
              <tr key={i} style={{ background: opp ? opportunityBg : undefined }}>
                <td style={{
                  ...ts.tdBase, color: opp ? SAND : palette.text, fontWeight: opp ? 600 : 500,
                ...
                <td style={{
                  ...
                  color: pos > 0 && pos <= 3 ? "#22c55e" : opp ? SAND : ts.tdBase.color, fontWeight: 600,
