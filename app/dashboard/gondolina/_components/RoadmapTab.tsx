...
import {
  ...
  ACCENT, ACCENT_SOFT, GOLD, POSITIVE,
  type Palette,
} from "./shared";
import { ContentPanel, fmtDay, parseDay, secondaryBtn, useNotionContent, type ContentState } from "./notionContent";
...
const BLUE = "#0ea5e9";
const VIEW_KEY = "pf.gondolina.roadmapView";

const OWNER_LABEL: Record<RoadmapOwner, string> = {
  pf: "Performance Flows",
  cliente: CLIENT_NAME,
  joint: "Insieme",
};
...
function statusColor(status: string | null, palette: Palette): string {
  ...
  if (status === "In corso") return GOLD;
...
export function RoadmapTab() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/gondolina/roadmap", { cache: "no-store" });
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <style>{`
        ...
        .pf-roadmap-clickable:focus-visible, .pf-roadmap-chip:focus-visible { outline: 2px solid ${ACCENT_SOFT}; outline-offset: 1px; }
...
function KanbanRow({ item, openId, contents, onToggle, onRetry }: { item: RoadmapItem } & RowProps) {
  ...
  const meta = [
    ...
    item.owner ? OWNER_LABEL[item.owner] : null,
...
function DetailChip({ isOpen }: { isOpen: boolean }) {
  ...
  return (
    <span style={{
      ...
      border: `1px solid ${ACCENT_SOFT}66`, background: isOpen ? `${ACCENT}29` : `${ACCENT}14`,
      color: palette.text, fontSize: 11, fontWeight: 600,
    }}>
      <svg width="12" height="12" fill="none" stroke={ACCENT_SOFT} strokeWidth="2" viewBox="0 0 24 24" aria-hidden="true">
        <path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z" /><polyline points="14 2 14 8 20 8" />
        <line x1="8" y1="13" x2="16" y2="13" /><line x1="8" y1="17" x2="13" y2="17" />
      </svg>
      {isOpen ? "Chiudi" : "Dettagli"}
      <svg width="10" height="10" fill="none" stroke={ACCENT_SOFT} strokeWidth="2.5" viewBox="0 0 24 24" aria-hidden="true"
