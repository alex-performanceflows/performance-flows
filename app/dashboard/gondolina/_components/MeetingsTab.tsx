...
import { useTheme, fmtDateTime, Card, CardHeader, EmptyState, Skeleton } from "./shared";
...
export function MeetingsTab() {
  ...
  const { contents, load: loadContent } = useNotionContent("/api/gondolina/meetings");

  const load = useCallback(async () => {
    setLoading(true); setFailed(false);
    try {
      const res = await fetch("/api/gondolina/meetings", { cache: "no-store" });
