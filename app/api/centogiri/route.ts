...
// Target: NEXT_PUBLIC_CENTOGIRI_ENDPOINT (leggibile anche server-side).
export const dynamic = "force-dynamic";
export const revalidate = 0;

export async function GET() {
  const endpoint = process.env.NEXT_PUBLIC_CENTOGIRI_ENDPOINT;
  if (!endpoint) {
    return NextResponse.json(
      { error: "NEXT_PUBLIC_CENTOGIRI_ENDPOINT non configurato" },
      { status: 500 }
    );
  }
  try {
